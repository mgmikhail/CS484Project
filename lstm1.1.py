...
scaler = MinMaxScaler()
df[['Close/Last']] = scaler.fit_transform(df[['Close/Last']])

# Extract closing prices and time deltas
closingPrices = df[['Close/Last', 'TimeDelta']].values

# Function to create sequences of past closing prices and time gaps
def create_sequences(data, seq_length):
    sequences, labels = [], []
    for i in range(len(data) - seq_length):
        sequences.append(data[i:i+seq_length])  # Last `seq_length` days of closing price + time gaps
...
# Split data into training and testing sets (40% test size for more evaluation)
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.4, shuffle=False)

# Reshape the data to fit LSTM input requirements
X_train = np.reshape(X_train, (X_train.shape[0], X_train.shape[1], 2))  # Now includes 'Close/Last' + 'TimeDelta'
X_test = np.reshape(X_test, (X_test.shape[0], X_test.shape[1], 2))

# Define the LSTM model
model = Sequential([
    LSTM(50, return_sequences=True, input_shape=(seqLength, 2)),  # Now taking 2 features per time step
    Dropout(0.3),
...
def predict_next_day(last_closing_prices, last_time_deltas):
    last_data = np.column_stack((last_closing_prices, last_time_deltas))
    last_data_scaled = scaler.transform(last_data)
    sequence = np.array([last_data_scaled[-seqLength:]]).reshape(1, seqLength, 2)
